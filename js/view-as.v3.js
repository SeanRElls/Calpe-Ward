/* =========================================================
   VIEW AS FEATURE - Admin impersonation (v3, cache-bust)
   ========================================================= */
...
async function startViewingAs(userId) {
  ...
  try {
    ...
    if (typeof loadUserPermissions === "function") { await loadUserPermissions(); }
    if (typeof populateShiftGrid === "function") { await populateShiftGrid(); }
...
async function populateViewAsSelector(targetSelector, statusEl) {
  ...
  const buildOptions = (users) => {
    const roles = { 1: "Charge Nurses", 2: "Staff Nurses", 3: "Nursing Assistants" };
    ...
    [1, 2, 3].forEach(roleId => {
      ...
      if (roleUsers.length) {
        ...
        roleUsers.forEach(u => { html += `<option value="${u.id}">${escapeHtml(u.name)}</option>`; });
        html += "</optgroup>";
      }
    });
    if (html === '<option value="">View As...</option>') {
      ...
      if (nonAdmin.length) {
        nonAdmin.forEach(u => { html += `<option value="${u.id}">${escapeHtml(u.name)}</option>`; });
      } else {
    ...
    selector.onchange = async function() {
      if (this.value) { await startViewingAs(this.value); } else { await stopViewingAs(); }
      this.value = "";
      updateViewAsBannerState();
    };
  };
  ...
  try {
    ...
    if (preloaded) { buildOptions(preloaded); return; }
...
function setupViewAsButton() {
  ...
  if (!btn) return;
  if (!currentUser || !currentUser.is_admin) { btn.style.display = "none"; return; }
  btn.style.display = "inline-flex";
  btn.addEventListener("click", () => {
    const banner = document.getElementById("viewAsBanner");
    if (banner && banner.style.display === "flex" && !getViewAsUser()) { hideViewAsBanner(); }
    else { showViewAsBanner(); }
  });
}
...
console.log("[VIEW AS] Module loaded v3");
