...
begin
  ...
  values (
    ...
    jsonb_build_object(
      ...
      'initiator_name', (select name from public.users where id = p_user_id),
...
begin
  ...
  -- If accepted, notify admins that swap is pending approval
...
begin
  ...
  select sr.id, sr.period_id,
    (select name from public.users where id = sr.initiator_user_id) as initiator_name,
    (select name from public.users where id = sr.counterparty_user_id) as counterparty_name,
...
begin
  ...
  select se.id, se.period_id,
    ...
    se.initiator_old_shift_date as initiator_date, se.initiator_old_shift_code as initiator_old_shift, se.initiator_new_shift_code as initiator_new_shift,
    se.counterparty_old_shift_date as counterparty_date, se.counterparty_old_shift_code as counterparty_old_shift, se.counterparty_new_shift_code as counterparty_new_shift,
...
begin
  ...
  return query
  select sr.id,
    u1.name::text as initiator_name,
    u2.name::text as counterparty_name,
    ...
    sr.created_at
  from swap_requests sr
  inner join public.users u1 on u1.id = sr.initiator_user_id
  inner join public.users u2 on u2.id = sr.counterparty_user_id
