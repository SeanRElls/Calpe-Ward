...
-- staff_request_shift_swap: drop (p_token, p_user_id, ...) overload
...
BEGIN
  ...
  WHERE user_id = v_uid AND date = p_initiator_shift_date AND status = 'published'
  LIMIT 1;
...
CREATE OR REPLACE FUNCTION public.admin_get_swap_executions(p_token uuid, p_period_id integer DEFAULT NULL::integer)
RETURNS TABLE(
  id uuid,
  period_id integer,
  initiator_name text,
